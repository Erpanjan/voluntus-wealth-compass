...
const ServiceFeatureGrid: React.FC<ServiceFeatureGridProps> = ({ services }) => {
  ...
  const getServiceIcon = (index: number) => {
    ...
    return <IconComponent className="w-8 h-8 stroke-1 text-brand-medium-gray" />;
  };
      ...
      <div className="container-custom mx-auto px-6 md:px-8 lg:px-0">
        <div className="flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
            {/* Investment Service */}
            <div className="bg-brand-almond rounded-3xl p-6 sm:p-8 min-h-[280px] flex flex-col shadow-soft hover:shadow-hover transition-all duration-300 border border-brand-silver/30">
              <div className="mb-6">
                {getServiceIcon(0)}
              </div>
              <div className="flex flex-col flex-grow">
                ...
                <p className="text-brand-medium-gray text-base leading-relaxed">
                  {services[0]?.content}
                </p>
              </div>
            </div>

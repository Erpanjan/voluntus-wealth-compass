      ...
      <div className="container-custom mx-auto px-6 md:px-8 lg:px-0">
        <div className="flex flex-col md:flex-row gap-12 lg:gap-16">
          ...
          <div className="md:w-2/5 lg:w-1/3">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-semibold mb-4 text-brand-accent">{title}</h2>
            ...
            <Accordion type="single" collapsible className="w-full space-y-4">
              {propositions.map((prop, index) => (
                <AccordionItem 
                  key={prop.id} 
                  value={prop.id}
                  className="border-none bg-brand-secondary rounded-xl overflow-hidden"
                >
                  <AccordionTrigger className="px-6 py-5 hover:no-underline">
                    <div className="flex flex-col items-start text-left">
                      <span className="text-xl font-semibold mb-1 text-brand-accent">{prop.title}</span>

...
const SectionCarousel: React.FC<SectionCarouselProps> = ({
  ...
  return <div className="relative h-full flex flex-col">
      ...
      <div className="flex justify-between items-center mb-10 mt-6">
        <h2 className="text-3xl md:text-4xl font-semibold">What We Can Help</h2>
        
        {/* Navigation numbers replaced dots */}
        <div className="flex gap-3 items-center">
          {sections.map((section, index) => <Button key={section.id} variant="ghost" size="sm" className={cn("w-6 h-6 p-0 flex items-center justify-center rounded-full transition-all duration-300", current === index ? "font-bold text-black" : "text-gray-400 font-normal hover:text-gray-600")} onClick={() => scrollToSection(index)} aria-label={`Go to section ${index + 1}`}>
              {index + 1}
            </Button>)}
        </div>
      </div>
      
      <div className="relative h-full flex-1" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onTouchStart={handleMouseEnter} onTouchEnd={handleMouseLeave}>
        ...
        dragFree: false
          ...
          <CarouselContent className="h-full">
            {sections.map((section, index) => <CarouselItem key={section.id} className="basis-full h-full flex items-center">

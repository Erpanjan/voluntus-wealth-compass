...
const NavLinks = ({ 
  ...
  return (
    <nav className="hidden lg:flex absolute left-1/2 transform -translate-x-1/2 items-center h-full">
      {navLinks.map((link) => (
        <Link
          key={link.name}
          to={link.path}
          onClick={(e) => handleNavLinkClick(e, isActive(link.path))}
          className={cn(
            "px-5 py-2 text-xs tracking-wide transition-all duration-300 flex items-center h-full",
            isActive(link.path) 

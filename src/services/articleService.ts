...
/**
 * Unified Article Service
 * Consolidates all article operations into a single, efficient service
 */
class ArticleService {
    ...
    if (typeof content === 'object') {
      ...
      for (const [key, value] of Object.entries(content)) {
        processed[key] = value === 'map[]' ? {} : 
          (typeof value === 'object' ? this.processContent(value) : value);
      }
      return processed;
    }
  ...
  async getPublishedArticlesByLanguage(
    ...
    if (!data || data.length === 0) return { articles: [], totalCount: 0 };
  ...
  async getMultilingualArticles(
    ...
    const articles = data.map((article: any): MultilingualArticle => ({
      ...
      content_en: this.processContent(article.content_en),
      content_zh: this.processContent(article.content_zh),
  ...
  /**
   * Get multilingual article by ID
   */
